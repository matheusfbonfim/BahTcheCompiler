...
class Scanner:
    def __init__(self, automato: Automaton):
        ...
        self._table = []                    # Tabela de tokens com o respectivo linha e coluna do token (cadeia, token, linha, coluna)

        self._erro_lexico = []              # Dicionario - Erro Lexico
    ...
    def analise_lexica(self):
        ...
        if valid_file:
            ...
            for linha_text in self._text:
                ...
                # Armazenando os erros lexicos - dicionario
                if len(erros_lexicos) > 0:
                    erros_lexicos.insert(0, f"[Erro Léxico] | Linha {num_linha} | ->") # Insere na casa 0 do vetor de erros
            ...
            # -----------------------
            # Caso tenha erro lexicos (Printar o primeiro erro de cada linha)
            if len(self._erro_lexico) > 0:
                print("Análise Lexica: [Gerando Erro]\n", end='')

                # Percorrendo os erros presentes na lista
                #   self._erro_lexico -> ['[Erro Léxico] | Linha 1 | ->', '\t Referência ..']
    ...
    def scanner(self, entrada_text, num_linha):
        ...
        # Estado inicial q0
        estado_atual_transitions = self._AF.get_transitions_state(0)  # Retorna dicionario de transições do estado
        ...
        while position < len(entrada_text):
            ...
            if caractere == "#" and not self._text_string:
                ...
                if self._AF.automato[estado_atual].is_end:
                    ...
                    self._table.append((cadeia, token, num_linha, position - len(cadeia) + 1))
        ...
        # Valida se o estado atual (depois de ler toda palavra) é final
        if self._AF.automato[estado_atual].is_end:
            token = self._AF.automato[estado_atual].token  # Captura o atributo token do estado
